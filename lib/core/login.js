 ...
 * 处理滑块验证码
 */
export async function slider (url, adapter) {
  try {
    /** 停止内置命令行适配器的监听 */
 ...
 */
export async function autoSlider (uid, url) {
  // 用户访问url
  const page = `https://hlhs-nb.cn/captcha/slider?key=${uid}`
 ...
 * 手动过滑块
 */
export async function manualSlider (uid, url) {
  logger.bot('info', uid, [
    '\n\n------------------------------------',
 ...
 * 处理设备锁
 */
export async function device (event, adapter) {
  try {
    /** 停止内置命令行适配器的监听 */
    process.stdin.emit('stdin.close')
    adapter.logger('info', [
